...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        res['withholding_journal_id'] = batch_result['payment_values']['withholding_journal_id']
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        res['withholding_tax_id'] = self.withholding_tax_id.id
        res['withholding_journal_id'] = self.withholding_journal_id.id
        res['withholding_amount'] = res['amount'] * (self.withholding_tax_id.amount / 100)
    ...
    def _create_payments(self):
        ...
        for payment in self:
            payment_lines = payment.line_ids.filtered_domain(
                domain) + res.withholding_move_id.line_ids.filtered_domain(domain) + res.line_ids.filtered_domain(
                domain)
            for account in payment_lines.account_id:
                if self.group_payment:
                    payment_lines.filtered_domain(
                        [('account_id', '=', account.id), ('reconciled', '=', True)]).remove_move_reconcile()

                    payment_lines.filtered_domain(
                        [('account_id', '=', account.id), ('reconciled', '=', False)]).reconcile()
                else:
                    for pay in res:
                        payment_lines = pay.withholding_move_id.line_ids.filtered_domain(
                            domain) + pay.line_ids.filtered_domain(
                            domain) + pay.reconciled_invoice_ids.line_ids.filtered_domain(
                            domain) + pay.reconciled_bill_ids.line_ids.filtered_domain(domain)
                        payment_lines.filtered_domain(
                            [('account_id', '=', account.id), ('reconciled', '=', True)]).remove_move_reconcile()
                        payment_lines.filtered_domain(
                            [('account_id', '=', account.id), ('reconciled', '=', False)]).reconcile()
    ...
    def _compute_payment_difference(self):
        for wizard in self:
            if not wizard.withholding_tax_id:
                ...
                wizard.payment_difference = total_amount_residual_in_wizard_currency - wizard.amount_net
            else:
