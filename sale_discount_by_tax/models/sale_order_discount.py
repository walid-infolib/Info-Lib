    ...
    def _create_discount_lines(self):
        ...
        if self.discount_type == "amount":
            total_price_per_tax_groups = defaultdict(
                float
            )  # Initialize a dictionary to store the total price_subtotal per tax group.
            ...
            if not total_price_per_tax_groups:
                ...
                taxes = next(
                    iter(total_price_per_tax_groups.keys())
                )  # Gives us the first tax found in the dict because there is only one

                subtotal = total_price_per_tax_groups[
                    taxes
                ]  # Gives us the sum of price before tax for each type of tax

                # Calculation of the discount percentage for each tax group
                discount_percentage_for_group = (
                    self.discount_amount
                    / (subtotal + subtotal * sum(taxes.mapped("amount")) / 100)
                ) * 100
                ...
                vals_list = []
                total_subtotal = sum(
                    total_price_per_tax_groups.values()
                )  # Sum of all perice subtotal same
                ...
                for tax, subtotal in total_price_per_tax_groups.items():
                    ...
                    amount_discount_for_group = (
                        self.discount_amount * weight
                    )  # Discount amount per tax group
