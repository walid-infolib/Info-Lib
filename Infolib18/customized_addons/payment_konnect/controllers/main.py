...
class KonnectController(http.Controller):
    ...
    def konnect_webhook(self, **raw_data):
        ...
        if not payment_ref:
            _logger.error("Payment provider reference missing in webhook data.")
        ...
        transaction = request.env['payment.transaction'].sudo().search([('provider_reference', '=', payment_ref)], limit=1)

        if transaction:
            # Process the notification data for the found transaction
            transaction._handle_notification_data('konnect', data)
        else:
            # Log an error if the transaction is not found
            _logger.error("No transaction found for payment payment_ref: %s", payment_ref)
