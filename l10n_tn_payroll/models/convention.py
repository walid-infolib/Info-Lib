...
class CollectiveAgreement(models.Model):
    ...
    category_ids = fields.One2many('collective.category', 'agreement_id', string='Categories')
...
class Category(models.Model):
    ...
    agreement_id = fields.Many2one('collective.agreement', string='Agreement', required=True)
    level_ids = fields.One2many('collective.level', 'category_id', string='Levels')

    @api.depends('name', 'agreement_id')
    def _compute_display_name(self):
        for category in self:
            category.display_name = category.name + ' (' + category.agreement_id.name + ')'
...
class Level(models.Model):
    ...
    display_name = fields.Char(compute="_compute_display_name")
    agreement_id = fields.Many2one('collective.agreement', string='Agreement', related="category_id.agreement_id",
                                   store=True)
    category_id = fields.Many2one('collective.category', string='Category', required=True)
...
class EmployeeAgreement(models.Model):
    ...
    level_id = fields.Many2one('collective.level', 'Agreement', required=True, ondelete="restrict")
