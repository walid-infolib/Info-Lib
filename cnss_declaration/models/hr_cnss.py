...
class HrCnss(models.Model):
    ...
    _order = "date desc"
    _description = (
        "The National Social Security Fund (CNSS) allows you to check "
    ...
    start_date = fields.Date(
        compute="_compute_quarter_year_date", string="start date", store=True
    )
    end_date = fields.Date(
        compute="_compute_quarter_year_date", string="end date", store=True
    )
    date = fields.Date(
        string="Date Of Declaration",
        help="Date Of Declaration",
    ...
    total_sum = fields.Monetary(
        string="Total sum", compute="_compute_total_sum", store=True
    )
    ...
    def generate_info_file(self):
        ...
        for line in self.line_ids:
            ...
            employee = f"{line.employee_id.name.upper():60}"
            if line.ssnid:
            ...
            if line.employee_id.identification_id:
                ...
                raise UserError(
                    _("Employee %s not have a identification number!")
                    % line.employee_id.name
                )
            salary = str(line.total).replace(".", "").zfill(10)
            blank_area = "".zfill(10)
            employee_ids.append(
                f"{num_employer}{exploit_code}{quarter}{year}{page}{line_num}"
        ...
        for record in self:
            if record.state == "done":
                raise ValidationError(
                    "You can not delete a confirmed CNSS declaration."
    ...
    def action_print_declaration_cnss(self):
        return self.env.ref(
            "cnss_declaration.report_cnss_declaration_template"
        ).report_action(self)
    ...
    def action_draft(self):
        if self.state == "done":
            ...
            attachments = self.env["ir.attachment"].search(
                [("res_id", "=", self.id), ("res_model", "=", "hr.cnss")]
            )
            attachments.unlink()
    ...
    def _check_cnss_declaration(self):
        for i in self:
            ...
            if declarations > 1:
                raise ValidationError(
                    _(
                        "You already have a declaration on this date of the quarter with this year"
                    )
                )
        ...
        for record in self:
            if record.quarter and record.year:
                dte = datetime.strptime(
                    "01/" + record.quarter + "/" + record.year, "%d/%m/%Y"
                )
    ...
    def _get_lines(self):
        if self.quarter and self.year and self.contract_type_id:
            self.line_ids = False
            date_month_start = datetime.strptime(
                str(self.start_date), "%Y-%m-%d"
            ).strftime("%b")[0]
            date_month_end = datetime.strptime(str(self.end_date), "%Y-%m-%d").strftime(
                "%b"
            )[0]
            payslip_data = self.env["hr.payslip"].read_group(
                domain=[
                    ...
                    ("state", "in", ["done", "paid"]),
                    ("contract_id.contract_type_id", "=", self.contract_type_id.id),
                ],
