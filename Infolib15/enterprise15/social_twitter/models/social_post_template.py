...
class SocialPostTemplate(models.Model):
    ...
    @api.depends(lambda self: ['message', 'image_ids', 'display_twitter_preview'] + self._get_post_message_modifying_fields())
    def _compute_twitter_preview(self):
        for post in self:
            if not post.display_twitter_preview:
                post.twitter_preview = False
                continue
