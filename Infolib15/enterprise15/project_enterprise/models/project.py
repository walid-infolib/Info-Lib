...
class Task(models.Model):
    ...
    def write(self, vals):
        ...
        # Check whether given dates are default values
        # if so, overwrite with schedule-based dates
        if (
            planned_date_begin
            and planned_date_begin + timedelta(days=+1, seconds=-1) == planned_date_end
            and not self.env.context.get('fsm_mode')
        ):
        ...
        res = super(Task, remaining).write(vals)
