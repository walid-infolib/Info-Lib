...
class TransferModelTestCase(AccountAutoTransferTestCase):
    ...
    def test_compute_transfer_lines_100_percent_transfer(self):
        ...
        self.assertEqual(lines.filtered(lambda l: l.account_id == self.company_data['default_account_revenue']).debit, 410.34)
        # 15% of the total amount
        self.assertEqual(lines.filtered(lambda l: l.account_id == slave_ids[0]).credit, 61.55)
        # 42.50% of the total amount
        self.assertEqual(lines.filtered(lambda l: l.account_id == slave_ids[1]).credit, 174.39)
        # the remaining amount of the total amount (42.50%)
        self.assertEqual(lines.filtered(lambda l: l.account_id == slave_ids[2]).credit, 174.4)
