...
import view_registry from 'web.view_registry';
    ...
    var PlanningCalendarController = CalendarController.extend({
        _onOpenCreate: function(event) {
            if (["day", "week"].includes(this.model.get().scale)) {
                event.options = event.options || {};
                event.options.context = event.options.context || {};
                event.options.context.planning_keep_default_datetime = true;
            }
