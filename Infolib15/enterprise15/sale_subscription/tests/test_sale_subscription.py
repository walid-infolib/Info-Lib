...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_upsell_retains_original_salesperson(self):
        test_user = self.env['res.users'].create({
            'name': 'Demo',
            'login': 'Demo',
        })
        test_team = self.env['crm.team'].create({
            'name': 'Sales Team',
        ...
        upsell_so_id = wiz.create_sale_order()['res_id']
        upsell_so = self.env['sale.order'].browse(upsell_so_id)
        self.assertEqual(upsell_so.user_id, test_user)
        res = subscription.prepare_renewal_order()
        renewal_so_id = res['res_id']
        renewal_so = self.env['sale.order'].browse(renewal_so_id)
        self.assertEqual(renewal_so.team_id, subscription.team_id)
