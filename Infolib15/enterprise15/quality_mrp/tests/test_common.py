...
class TestQualityMrpCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.product_product_drawer_drawer = self.env['product.product'].create({
            'name': 'Drawer Black',
            'tracking': 'lot'
        })
        ...
        self.bom = self.env['mrp.bom'].create({
            ...
            'bom_line_ids': [
                (0, 0, {
                    'product_id': self.product_product_drawer_drawer.id,
                    'product_qty': 1,
                    'product_uom_id': self.product_uom_id,
                    'sequence': 1,
                }), (0, 0, {
        ...
        self.lot_product_product_drawer_drawer_0 = self.env['stock.production.lot'].create({
            'name': '0000000010001',
            'product_id': self.product_product_drawer_drawer.id,
            'company_id': self.env.company.id,
        })
        self.lot_product_product_drawer_case_0 = self.env['stock.production.lot'].create({
            'name': '0000000020045',
            'product_id': product_product_drawer_case.id,
            'company_id': self.env.company.id,
        })
