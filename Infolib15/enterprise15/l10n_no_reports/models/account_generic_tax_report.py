...
class AccountGenericTaxReport(models.AbstractModel):
    ...
    def _l10n_no_query_taxes(self, options):
        ...
        self._cr.execute(f'''
            ...
            JOIN account_tax_report_line_tags_rel report_line_rel ON report_line_rel.account_account_tag_id = tag.id
            JOIN account_tax_report_line report_line ON report_line.id = report_line_rel.account_tax_report_line_id
            JOIN res_country tax_country ON tax_country.id = tax.country_id
            WHERE tdr.tax_exigible AND tax_country.code = 'NO'
    ...
    def get_xml(self, options):
        ...
        tax_details_dict = self._l10n_no_query_taxes(options)
