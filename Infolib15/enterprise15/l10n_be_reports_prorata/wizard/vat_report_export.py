...
class AccountFinancialReportXMLReportExport(models.TransientModel):
    ...
    prorata = fields.Integer(string="Definitive pro rata")
    ...
    submit_more = fields.Boolean(string="I want to submit more than 5 specific pro rata")
    ...
    def _compute_show_prorata(self):
        date_to = fields.Date.to_date(self.env.context.get('date_to') or self.env.context.get('l10n_be_reports_closing_date'))
        if date_to and (
    ...
    def _l10n_be_reports_vat_export_generate_options(self):
        ...
        if self.is_prorata_necessary:
            if not re.match(r'\d{4}', self.prorata_year) or int(self.prorata_year) < 2000:
                raise UserError(_("Please enter a valid pro rata year (more than 2000)"))
