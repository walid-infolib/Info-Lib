...
tour.register('test_barcode_batch_scan_lots', {test: true}, [
    ...
    // Now scan 1x lot2. It should create a new line for receipt_1 still the line is uncomplete.
    ...
    { trigger: '.o_barcode_line.o_selected:not(.o_line_completed)', run: 'scan lot2' },
    {
        ...
        run: function () {
            const $lines = helper.getLines();
            const [$line1, $line2] = [$($lines[0]), $($lines[1])];
            helper.assertLineBelongTo($line1, "receipt_2");
            helper.assertLineBelongTo($line2, "receipt_1");
    ...
    { trigger: '.o_barcode_client_action', run: 'scan lot3' },
    {
        ...
        run: function () {
            ...
            helper.assertLineBelongTo($line1, "receipt_2");
            helper.assertLineBelongTo($subline1, "receipt_2");
            helper.assertLineBelongTo($subline1, "receipt_2");
            helper.assertLineBelongTo($line2, "receipt_1");
            helper.assertLineQty($line1, "4");
    ...
    { trigger: '.o_barcode_line.o_selected', run: 'scan lot2' },
    {
        ...
        run: function () {
            ...
            helper.assertLineBelongTo($line1, "delivery_2");
            helper.assertLineBelongTo($line2, "delivery_1");
            helper.assertLineBelongTo($subline1, "delivery_1");
            helper.assertLineBelongTo($subline2, "delivery_1");
            helper.assertLineQty($line1, "0");
            helper.assertLineQty($line2, "3");
            helper.assertLineQty($subline1, "1");
            helper.assertLineQty($subline2, "2");
            helper.assert($subline1.find('.o_line_lot_name').text(), 'lot3');
            helper.assert($subline2.find('.o_line_lot_name').text(), 'lot2');
        },
    },
    ...
    { trigger: '.o_barcode_client_action', run: 'scan lot3' },
    {
        ...
        run: function () {
            const $lines = helper.getLines();
            const [$line1, $line2] = [$($lines[0]), $($lines[4])];
            const [$subline1, $subline2, $subline3] = [$($lines[1]), $($lines[2]), $($lines[3])];
            helper.assertLineBelongTo($line1, "delivery_2");
            helper.assertLineBelongTo($subline1, "delivery_2");
            helper.assertLineBelongTo($subline1, "delivery_2");
            helper.assertLineBelongTo($subline3, "delivery_2");
            helper.assertLineBelongTo($line2, "delivery_1");
            helper.assertLineQty($line1, "3");
            helper.assertLineQty($subline1, "1");
            ...
            helper.assert($subline3.find('.o_line_lot_name').text(), 'lot2');
        },
    },
    { trigger: '.o_barcode_client_action', run: 'scan O-BTN.validate' },
    { trigger: '.o_notification.bg-success' },
