...
#        })
    ...
    def test_flexible_consumption_1c(self):
        ...
        self.assertEqual(checks_without_quality_point[0].title, '{} "{}"'.format(checks_without_quality_point[0].test_type_id.display_name, wo_form.component_id.name), "title of quality check with no control point should be based on consumed material test type + component")
        wo._next()
        wo.do_finish()
