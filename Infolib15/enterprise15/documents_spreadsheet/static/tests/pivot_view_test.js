...
test("Adding a pivot invalidates the evaluation", async function (assert) {
    assert.expect(4);
    const { model } = await createSpreadsheetFromPivot();
    await waitForEvaluation(model);
    assert.equal(getCellValue(model, "C3"), 15);
    setCellContent(model, "C22", '=PIVOT("2","probability","bar","false","foo","2")');
    await waitForEvaluation(model);
    ...
    assert.equal(getCellValue(model, "C22"), "Loading...");
    await waitForEvaluation(model);
    assert.equal(getCellValue(model, "C22"), 15);
});
