...
class AEATAccountFinancialReport(models.Model):
    ...
    def _generate_mod_303_page1(self, options, current_company, period, year, casilla_lines_map):
        ...
        if options['date']['date_from'] >= '2023-01-01':
            ...
            rslt += self._boe_format_number(casilla_154, length=5)
        ...
        rslt += self._boe_format_number(casilla_17, length=5)
        ...
        # Footer of page 1
        rslt += self._boe_format_string(' ' * reserved_empty_chars)  # Reserved for AEAT
        rslt += self._boe_format_string(' ' * 13)  # Reserved for AEAT
        rslt += self._boe_format_string('</T30301000>')
    ...
    def _generate_mod_303_page3(self, options, current_company, period, year, casilla_lines_map):
        ...
        if options['date']['date_to'] >= '2024-09-30':
            rslt += self._boe_format_string('rectification_direct_debit' in boe_wizard_fields and boe_wizard.rectification_direct_debit and 'X' or ' ')
            rslt += self._boe_format_number(casilla_lines_map.get('108', 0), length=17, decimal_places=2, signed=True, in_currency=True)
            rslt += self._boe_format_number(casilla_lines_map.get('111', 0), length=17, decimal_places=2, in_currency=True)
            rslt += self._boe_format_string(' ' * 120)
            rslt += self._boe_format_string('rectification_motive_rectifications' in boe_wizard_fields and boe_wizard.rectification_motive_rectifications and 'X' or ' ')
            rslt += self._boe_format_string('rectification_motive_discrepancy_adm_crit' in boe_wizard_fields and boe_wizard.rectification_motive_discrepancy_adm_crit and 'X' or ' ')
