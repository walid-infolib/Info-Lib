...
class AccountMove(models.Model):
    ...
    def _onchange_partner_id(self):
        if self.move_type == "out_invoice":
            tfs = self.env["ir.model.data"].search(
                [("name", "ilike", "l10n_tn_tax_vat_sale_tax_stamp")]
            )
            stamp_tax = self.env["account.tax"].browse(tfs.res_id)
        elif self.move_type == "in_invoice":
            tfp = self.env["ir.model.data"].search(
                [("name", "ilike", "l10n_tn_tax_vat_purchase_tax_stamp")]
            )
